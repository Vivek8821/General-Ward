...
- [[_COMMUNITY_Pharmacy & Inventory UI|Pharmacy & Inventory UI]]
- [[_COMMUNITY_Auth, Audit & Admin Security|Auth, Audit & Admin Security]]
- [[_COMMUNITY_Enterprise Hardening & DB Architecture|Enterprise Hardening & DB Architecture]]
- [[_COMMUNITY_Launch Monitoring & Feature Flags|Launch Monitoring & Feature Flags]]
- [[_COMMUNITY_Dashboard & Patient UI Components|Dashboard & Patient UI Components]]
...
- [[_COMMUNITY_Medication Service|Medication Service]]
- [[_COMMUNITY_Tenant Middleware|Tenant Middleware]]
- [[_COMMUNITY_Waste Repository|Waste Repository]]
- [[_COMMUNITY_Stock Repository|Stock Repository]]
- [[_COMMUNITY_Observation & EWS Service|Observation & EWS Service]]
- [[_COMMUNITY_Clinical Audit Service|Clinical Audit Service]]
- [[_COMMUNITY_Waste Service|Waste Service]]
- [[_COMMUNITY_Batch Service & Recall|Batch Service & Recall]]
- [[_COMMUNITY_Barcode Repository|Barcode Repository]]
- [[_COMMUNITY_Purchase Order Repository|Purchase Order Repository]]
- [[_COMMUNITY_Patient Detail UI & Design Tokens|Patient Detail UI & Design Tokens]]
- [[_COMMUNITY_Config & Environment|Config & Environment]]
- [[_COMMUNITY_DB Adapter (SQLite & Postgres)|DB Adapter (SQLite & Postgres)]]
- [[_COMMUNITY_Auth Lockout Repository|Auth Lockout Repository]]
- [[_COMMUNITY_Escalation Repository|Escalation Repository]]
- [[_COMMUNITY_Task Repository|Task Repository]]
- [[_COMMUNITY_Pharmacy Reorder Service|Pharmacy Reorder Service]]
- [[_COMMUNITY_Pharmacy Analytics Service|Pharmacy Analytics Service]]
- [[_COMMUNITY_Barcode Service|Barcode Service]]
- [[_COMMUNITY_Task Service|Task Service]]
- [[_COMMUNITY_Stock Service|Stock Service]]
- [[_COMMUNITY_Codemap Generation Scripts|Codemap Generation Scripts]]
- [[_COMMUNITY_Admin Audit View|Admin Audit View]]
...
- [[_COMMUNITY_Handover Notes Repository|Handover Notes Repository]]
...
- [[_COMMUNITY_Logger Internals|Logger Internals]]
- [[_COMMUNITY_Medication Validation|Medication Validation]]
- [[_COMMUNITY_Auth Repository|Auth Repository]]
- [[_COMMUNITY_Migration Service|Migration Service]]
- [[_COMMUNITY_Auth Service|Auth Service]]
- [[_COMMUNITY_Report Verification Service|Report Verification Service]]
...
- [[_COMMUNITY_Community 141 – ward-backendmiddlewarerbac.j|Community 141 – ward-backend/middleware/rbac.j]]
- [[_COMMUNITY_Community 142 – ward-frontendsrcviewsTasks.|Community 142 – ward-frontend/src/views/Tasks.]]
- [[_COMMUNITY_Community 143 – ward-frontendsrcviewsAdminA|Community 143 – ward-frontend/src/views/AdminA]]
- [[_COMMUNITY_Community 144 – Ward Management HTML Prototype|Community 144 – Ward Management HTML Prototype]]
- [[_COMMUNITY_Community 145 – CSRF Protection Middleware|Community 145 – CSRF Protection Middleware]]
- [[_COMMUNITY_Community 146 – Stress Test Pass Criteria|Community 146 – Stress Test Pass Criteria]]
- [[_COMMUNITY_Community 147 – Config Key PORT|Community 147 – Config Key: PORT]]

## God Nodes (most connected - your core abstractions)
1. `useAuth()` - 29 edges
2. `CODEMAP.md (Repository Codemap)` - 25 edges
3. `Ward Backend Architectural Navigation` - 18 edges
4. `Database Adapter` - 15 edges
5. `Migration 002: Application Schema` - 15 edges
6. `ward-backend CODENAV.md` - 14 edges
7. `PDFReportService` - 13 edges
8. `Signup & Payment Integration Detailed Plan` - 13 edges
9. `BatchRepository` - 12 edges
10. `Launch Monitoring & Contingency Detailed Plan` - 12 edges

## Surprising Connections (you probably didn't know these)
- `ward-management.html (Legacy SPA prototype)` --semantically_similar_to--> `Tenant Isolation (multi-tenant data partitioning)`  [AMBIGUOUS] [semantically similar]
  ward-management.html → cursorrules.md
- `doctor_cookies.txt (Dr. Smith JWT cookie)` --references--> `ward-backend/middleware/auth.js`  [INFERRED]
  doctor_cookies.txt → ward-backend/middleware/auth.js
- `admin_cookies.txt (Ward Admin JWT cookie)` --references--> `ward-backend/middleware/auth.js`  [INFERRED]
  admin_cookies.txt → ward-backend/middleware/auth.js
- `cookies.txt (Dr. Smith JWT cookie, older)` --references--> `ward-backend/middleware/auth.js`  [INFERRED]
  cookies.txt → ward-backend/middleware/auth.js
...
Cohesion: 0.05
Nodes (61): admin_cookies.txt (Ward Admin JWT cookie), AuditLogs (SQLite table), ward-backend/routes/adminAudit.js, ward-backend/middleware/audit.js, ward-backend/middleware/auth.js, ward-backend/controllers/AuthController.js, ward-backend/repositories/AuthRepository.js, ward-backend/services/AuthService.js (+53 more)

### Community 1 - "Auth, Audit & Admin Security"
Cohesion: 0.07
Nodes (36): Alert Engine, Clinical Workflow Milestone Tracking, Existing Compliance Capabilities, HttpOnly Cookie + CSRF Double-Submit Auth, Helmet CSP Configuration, CSRF Enforcement (Cookie Auth Mutations), DB Adapter (SQLite/Postgres Abstraction), Dev Credential Gating (+28 more)

### Community 2 - "Enterprise Hardening & DB Architecture"
Cohesion: 0.09
...
Cohesion: 0.21
Nodes (7): getMigrationsDir(), listMigrationFiles(), planMigrations(), seed(), getArg(), run(), seed()

### Community 18 - "Clinical Audit Service"
Cohesion: 0.18
Nodes (11): Clinical Grade Stability Principle, Concurrency Hardening via SQLite WAL, System Architect Directives (cursorrules), HospitalArchives Immutable Discharge Snapshots, HospitalArchives Table, IMPLEMENTATION_STATE.json (Crash Recovery State), JWT Authentication, Layered Architecture Pattern (Controllers->Services->Repositories) (+3 more)
...
- `ward-management.html (Legacy SPA prototype)` → `Tenant Isolation (multi-tenant data partitioning)`  [AMBIGUOUS]
  ward-management.html · relation: semantically_similar_to
...
- **Are the 10 inferred relationships involving `Database Adapter` (e.g. with `Polymorphic DB Adapter (SQLite/Postgres)` and `Config Key: DB_DIALECT`) actually correct?**
  _`Database Adapter` has 10 INFERRED edges - model-reasoned connections that need verification._
- **What connects `doctor_cookies.txt (Dr. Smith JWT cookie)`, `admin_cookies.txt (Ward Admin JWT cookie)`, `cookies.txt (Dr. Smith JWT cookie, older)` to the rest of the system?**
